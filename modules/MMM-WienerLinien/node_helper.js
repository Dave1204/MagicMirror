...
module.exports = NodeHelper.create({
    ...
    async getData() {
        ...
        if (this.config.elevatorStations.length > 0) {
            const url = `${
                this.baseUrl
            }/trafficInfoList?name=aufzugsinfo&relatedStop=${this.config.elevatorStations.join(
                "&relatedStop="
            )}`;
        ...
        if (this.config.incidentLines.length > 0) {
            const incidentType = this.config.incidentShort
                ? "stoerungkurz"
                : "stoerunglang";
            const url = `${
                this.baseUrl
            }/trafficInfoList?name=${incidentType}&relatedLine=${this.config.incidentLines.join(
                "&relatedLine="
            )}`;
    ...
    async getMonitoringData() {
        try {
            const response = await fetch(
                `${this.baseUrl}/monitor?rbl=${this.config.stations.join(
                    "&rbl="
                )}`
            );
            ...
            if (parsedBody.message.value === "OK") {
                this.handleData(
                    parsedBody.data.monitors,
                    parsedBody.message.serverTime
                );
            } else {
    ...
    handleData(data, time) {
        ...
        for (let i = 0; i < data.length; i += 1) {
            if (
                !Object.prototype.hasOwnProperty.call(
                    stations,
                    data[i].locationStop.properties.name
                )
            ) {
            ...
            for (let n = 0; n < data[i].lines.length; n += 1) {
                ...
                for (
                    let x = 0;
                    x < data[i].lines[n].departures.departure.length;
                    x += 1
                ) {
                    if (
                        Object.keys(
                            data[i].lines[n].departures.departure[x]
                                .departureTime
                        ).length === 0
                    ) {
                        metroFlag = true;
                ...
                if (metroFlag) {
                    ...
                    const departureTimeMatches = data[i].lines[n].towards
                        .match(departureTimePattern)
                        .toString()
                        .split(",");

                    const towardsPattern = /^[a-zäöüß ]+/i;
                    const towardsMatch = data[i].lines[n].towards
                        .match(towardsPattern)
                        .toString()
                        .replace(/ {2,}/g, " ")
                        .trim();
                    ...
                    for (let x = 0; x < departureTimeMatches.length; x += 1) {
                        ...
                        datetime.setMinutes(
                            datetime.getMinutes() + departureTimeMatches[x]
                        );
        ...
        for (let i = 0; i < keys.length; i += 1) {
            stations[keys[i]].departures = stations[keys[i]].departures.sort(
                (a, b) => {
