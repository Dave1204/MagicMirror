...
class BringClient {
  ...
  constructor(opts, modulePath) {
    ...
    this.store = new Store({
      path: modulePath + "/bring.config.json",
      cwd: "store"
    });
    this._init(opts);
  }
  ...
  _init(opts) {
    ...
    if (this.mustLogin()) {
      this.login(opts.email, opts.password).then(() => {
        ...
        this.#authenticatedClient.defaults.headers.common["Authorization"] =
          "Bearer " + this.store.get("access_token");
      });
    } else {
      this.userId = this.store.get("user_id");
      this.defaultListId = this.store.get("default_list_id");
  ...
  mustLogin() {
    return (
      !this.store.get("access_token") ||
      !this.store.get("valid_until") ||
      new Date().getTime() > this.store.get("valid_until")
    );
  }
  ...
  login(email, password) {
    return this.#authenticatedClient
      ...
      .then((response) => {
        ...
        this.store.set(
          "valid_until",
          new Date().getTime() + loginObj["expires_in"] * 1000
        );
      })
      .catch((error) => {
        console.error(
          "Error while Logging-in with Bring-Client in MMM-Bring:",
  ...
  getLists() {
    return this.#authenticatedClient
      .get("/bringusers/" + this.userId + "/lists")
      .then((response) => {
        // Remember the names of the Lists
        for (let i = 0, len = response.data.lists.length; i < len; i++) {
          this.store.set(
            "list_name_" + response.data.lists[i].listUuid,
            response.data.lists[i].name
          );
          this.store.set(
            "list_id_" + response.data.lists[i].name,
            response.data.lists[i].listUuid
          );
        }
  ...
  getImageSrc(item) {
    ...
    if (
      ...
      const articleKeys = Object.keys(this.articles).map((key) =>
        key.toLowerCase()
      );
    ...
    return (
      "https://web.getbring.com/assets/images/items/" +
      name
        .replace(/[.*+-?^${}()|/[\]\\]/g, "_")
        .replace(/&/g, "_")
        .replace(/\s/g, "_")
